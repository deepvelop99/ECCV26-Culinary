...
class RecordEpisode(gym.Wrapper):
    ...
    def capture_image(self, infos=None):
        if self.save_camera == "main":
            img = self.env.render()
            img = common.to_numpy(img)
            # Squeeze batch dim for single-env renders (shape (1,H,W,C) → (H,W,C)).
            if img.ndim == 4 and img.shape[0] == 1:
                img = img[0]
            if img.dtype != np.uint8:
                img = (img * 255).clip(0, 255).astype(np.uint8)
        else:
            self.env.base_env.capture_sensor_data()
            images = self.env.get_sensor_images()

            if self.save_camera not in images:
                raise ValueError(f"Camera {self.save_camera} not found in get_sensor_images()")

            cam_data = images[self.save_camera]
            # ✅ depth, seg 같은 건 무시하고 rgb만 사용
            if "rgb" not in cam_data:
                raise ValueError(f"Camera {self.save_camera} has no rgb output")

            img = cam_data["rgb"]
            # print(img.shape)
            if hasattr(img, "cpu"):  # torch.Tensor → numpy
                img = img.cpu().numpy()
