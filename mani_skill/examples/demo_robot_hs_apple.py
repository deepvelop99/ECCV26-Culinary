...
import gymnasium as gym
import mani_skill.envs  # 등록
ids = sorted([k for k in gym.envs.registry.keys() if "RoboCasa" in k])
...
def main():
    ...
    p = Path("~/.maniskill/data/scene_datasets/robocasa_dataset/assets/objects/objaverse/apple/apple_10/visual/model_normalized_0.obj").expanduser()
    assert p.exists(), f"OBJ not found: {p}"
    builder.add_visual_from_file(str(p), scale=[0.1, 0.1, 0.1])  # 50%로 축소
    obj_vis = builder.build_static(name="obj_vis")  # 충돌체 없음 → 통과
    obj_vis.set_pose(sapien.Pose([3, -3.2, 0.96]))  # Z-up, 단위 m
