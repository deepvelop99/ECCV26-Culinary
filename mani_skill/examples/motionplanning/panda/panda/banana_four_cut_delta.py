...
def main(args: Args):
    ...
    delta = object_width / 4.0  # 분할 단위
    target_ys = [
        object_center_y - 0.09,
        object_center_y - 0.04,
        object_center_y + 0.03,
        object_center_y + 0.07,
    ]
    print(f"[TARGET Y] {target_ys}")
    rand_x_variation=-0.2
    # rand_x_variation = random.uniform(-0.35, -0.2)
    rand_approach_z = random.uniform(0.3, 0.5)
    # randomspeed = random.randint(10, 100)
    randomspeed =10
    # --- 절삭 루프 ---
    for i, target_y in enumerate(target_ys):
        print(f"[Cutting {i+1}/{len(target_ys)}] Target y={target_y:.3f}")

        y_off = target_y - object_center_y
        approach_pose = sapien.Pose(p=pose_p + np.array([rand_x_variation, y_off, rand_approach_z], dtype=np.float32))
        cut_pose      = sapien.Pose(p=pose_p + np.array([rand_x_variation, y_off, -0.17], dtype=np.float32))
        ...
        # move_to_pose(env, init_pose_base, approach_pose_base, steps=randomspeed, gripper=CLOSED)
        # move_to_pose(env, approach_pose_base, cut_pose_base, steps=randomspeed, gripper=CLOSED)
        # move_to_pose(env, cut_pose_base, retreat_pose_base, steps=randomspeed, gripper=CLOSED)
        move_to_pose(env, init_pose_world, approach_pose, steps=100, kp=20.0, clip=0.1, gripper_delta=0.0)
        move_to_pose(env, approach_pose,    cut_pose,    steps=randomspeed, kp=30.0, clip=0.05, gripper_delta=0.0)
        move_to_pose(env, cut_pose,         retreat_pose,steps=100, kp=20.0, clip=0.1, gripper_delta=0.0)
