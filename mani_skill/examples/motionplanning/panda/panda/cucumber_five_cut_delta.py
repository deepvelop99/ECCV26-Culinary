...
def main(args: Args):
    ...
    delta = object_width / 5.0  # 분할 단위
    target_ys = [
        object_center_y - 0.1,
        object_center_y - 0.07,
        object_center_y + 0.01,
        object_center_y + 2.0 * delta - 0.035,
        object_center_y + 2.0 * delta + 0.005,
    ]
    print(f"[TARGET Y] {target_ys}")
    rand_x_variation = -0.2
    # rand_x_variation = random.uniform(-0.25, -0.2)
    ...
    for i, target_y in enumerate(target_ys):
        ...
        cut_pose      = sapien.Pose(p=pose_p + np.array([rand_x_variation, y_off, 0.12], dtype=np.float32))
        ...
        move_to_pose(env, approach_pose,    cut_pose,    steps=randomspeed, kp=20.0, clip=0.1, gripper_delta=0.0)
