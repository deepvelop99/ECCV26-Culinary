...
class PandaKnifeMotionPlanningSolver:
    ...
    def _compose_action(self, qpos: np.ndarray, qvel: np.ndarray = None):
        ...
        elif self.control_mode == "pd_ee_pos_vel":
            # 10차원 EEF action: pose(6) + vel(3) + gripper(1)
            # 여기서는 planner가 joint trajectory만 내주므로,
            # 일단 delta=0, vel=0 넣고 gripper만 붙여줌
            core = np.zeros(9, dtype=np.float32)
        else:
            core = qpos
