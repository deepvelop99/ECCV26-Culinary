...
def main(args: Args):
    ...
    output_dir = f"{args.record_dir}/{args.env_id}/teleop{seed}_{random.randint(1000, 9999)}/"
    print(output_dir)
    ...
    env = gym.make(
        ...
        control_mode="pd_joint_pos",  # ⬅️ 이 부분을 수정했습니다.
        render_mode="human",
        reward_mode="none",
        enable_shadow=False,
    ...
    # names = [j.get_name() for j in env.agent.robot.get_active_joints()]
    # print("ACTIVE JOINTS:", names)         # 순서 중요!
    # print("QPOS SHAPE:", env.agent.robot.get_qpos().shape)
    # print("FIRST QPOS:", env.agent.robot.get_qpos())
    ...
    if args.save_video:
        ...
        env = gym.make(
            ...
            control_mode="pd_joint_pos", # ⬅️ 비디오 재생 시에도 컨트롤러 모드를 일치시킵니다.
            render_mode="human",
            reward_mode="none",
            enable_shadow=True,
...
def solve(env: BaseEnv, debug=False, vis=False):
    ...
    # ----------------------------------------------------------- #
    assert env.unwrapped.control_mode in ["pd_ee_pose", "pd_joint_pos", "pd_joint_pos_vel"]
    ...
    elif env.unwrapped.robot_uids in ["panda", "panda_wristcam","pkour"]:
...
# gizmo를 scene에서 찾아서 hidden 처리
    for plugin in viewer.plugins:
        try:
            import sapien.utils.viewer.viewer as vmod
            if isinstance(plugin, vmod.TransformWindow):
                transform_window = plugin
                if hasattr(transform_window, "_gizmo"):
                    env.unwrapped._hidden_objects.append(transform_window._gizmo)
                break
        except Exception:
            pass
    ...
    def select_panda_tcp():
        tcp_link = sapien_utils.get_obj_by_name(env.agent.robot.links, "panda_hand_tcp")
    ...
    while not viewer.closed:
        ...
        if execute_current_pose:
            ...
            result = None
            if env.unwrapped.robot_uids in ["pk", "panda_stick","pkour"]:
