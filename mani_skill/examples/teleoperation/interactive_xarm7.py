...
from mani_skill.examples.motionplanning.xarm7.motionplanner import (
    XArm7AbilityMotionPlanningSolver,
)
...
class Args:
    ...
    """Supported: panda, panda_wristcam, panda_stick, pk (knife), xarm6_robotiq, xarm7_ability"""
...
def solve(env: BaseEnv, debug=False, vis=False):
    ...
    if env.unwrapped.robot_uids in ["panda_stick", "pk","pkhs"]:
    ...
    elif env.unwrapped.robot_uids == "xarm6_robotiq":
        ...
        robot_has_gripper = True
    elif env.unwrapped.robot_uids == "xarm7_ability":
        planner = XArm7AbilityMotionPlanningSolver(
            env, debug=debug, vis=vis,
            base_pose=env.unwrapped.agent.robot.pose,
            visualize_target_grasp_pose=False,
            print_env_info=False,
    ...
    def select_panda_tcp():
        # Try to find tcp link, if not found try other end-effector names
        # For xarm7_ability, use link7 to match the planner's move_group
        if env.unwrapped.robot_uids == "xarm7_ability":
            tcp_candidates = ["link7", "base"]
        else:
            tcp_candidates = ["tcp", "base", "link7"]

        tcp_link = None
        for candidate in tcp_candidates:
            tcp_link = sapien_utils.get_obj_by_name(env.agent.robot.links, candidate)
            if tcp_link is not None:
                print(f"Selected TCP link: {candidate}")
                break
    ...
    while not viewer.closed:
        ...
        if viewer.window.key_press("h"):
            ...
            print("Key pressed: n - executing pose")
        ...
        if execute_current_pose:
            ...
            result = None
            if env.unwrapped.robot_uids in ["pk", "pkhs"]:
