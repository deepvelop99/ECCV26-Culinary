...
from transforms3d.quaternions import qmult, rotate_vector, quat2mat
...
@register_env("applecut", max_episode_steps=1000)
class TableTopCuttingEnv(BaseEnv):
    ...
    def _load_scene(self, options: dict):
        ...
        for env_idx in range(self.num_envs):
            ...
            path = "./mani_skill/assets/fruits/apple.obj" # banana
            ...
            obj_builder.add_sphere_collision(radius=1e-100000000)
        ...
        self.update_block_rot_quat = euler2quat(np.pi/2, 0, np.pi/2) # apple update rotation
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        for e in env_idx.tolist():
            ...
            self.init_quat = self.block_rot_quat  

            # --- 위치만 랜덤화 ---
            self.rand_x = self.board_center[0] + np.random.uniform(-0.03, 0.03)
            self.rand_y = self.board_center[1] + np.random.uniform(-0.1, 0.1)
            self.rand_z = self.board_center[2]  # 보드 위

            # 초기 quat 그대로 유지
            self.block_apple[e].set_pose(
                Pose.create_from_pq([self.rand_x, self.rand_y, self.rand_z], self.init_quat)
            )
    ...
    def _after_control_step(self):
        ...
        cut_paths = [
            "./mani_skill/assets/fruits/update/apple/apple_left.obj",
            "./mani_skill/assets/fruits/update/apple/apple_right.obj",
        ]
        ...
        for e in range(self.num_envs):
            ...
            reached_board = tip_z <= board_z + 0.07  # 작은 margin 허용
            ...
            if reached_board and (not self._already_cut[e]):
                ...
                for cut_path in cut_paths:
                    ...
                    # --- 조각별 랜덤 오프셋 적용 ---
                    dy = random.uniform(0.02, 0.025)   # y축 이동량 (좌우)
                    pose_p[0]-=0.005 
                    pose_p[1] -= 0.0015
                    ...
                    # --- 회전 (Z축 기준 90도) ---
                    rot_z_90 = euler2quat(0, 0, np.pi / 2)
                    new_q = qmult(rot_z_90, pose_q)

                    cut_actor.set_pose(Pose.create_from_pq(pose_p, new_q))
                    new_pieces.append(cut_actor)
                    # cut_actor.set_pose(Pose.create_from_pq([self.rand_x, (self.rand_y-0.01), self.rand_z], self.update_block_rot_quat))
    ...
    def _ensure_buffers(self):
        ...
        if not isinstance(getattr(self, "_already_cut", None), np.ndarray):
            self._already_cut = np.zeros(num, dtype=bool)
        elif self._already_cut.shape[0] != num:
            # shape만 맞추고 값 유지
            old = self._already_cut
            self._already_cut = np.zeros(num, dtype=bool)
            self._already_cut[:min(num, len(old))] = old[:min(num, len(old))]
    ...
    def _get_actor_aabb(self, actor):
        ...
        if mesh_path is None or not os.path.exists(mesh_path):
            mesh_path = "./mani_skill/assets/fruits/apple.obj"
