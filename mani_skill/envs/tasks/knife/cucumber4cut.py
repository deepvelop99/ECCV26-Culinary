...
@register_env("cucumber4cut", max_episode_steps=1000)
class TableTopCuttingEnv(BaseEnv):
    ...
    def _load_scene(self, options: dict):
        ...
        for env_idx in range(self.num_envs):
            ...
            # path = "./mani_skill/assets/fruits/update/banana/multicut/4cut/banana_4cut_final.obj"
            ...
            self.block_apple.append(builder.build(name=f"block_cucmber_{env_idx}"))
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        for e in env_idx.tolist():
            ...
            rand_x = self.board_center[0] + np.random.uniform(0.05, 0.2)
            rand_y = self.board_center[1] + np.random.uniform(-0.2, 0.2)
    ...
    def _after_control_step(self):
        ...
        for e in range(self.num_envs):
            ...
            inside_tip = (
                (tip_p[e, 1]+ 0.05 > (pose_p[1] - half[2])) & (tip_p[e, 1]+0.05 < (pose_p[1] + half[2])) &
            ...
            delta = object_width / 4.0
            one_target_y  = object_center_y + 0.03
            two_target_y  = one_target_y  + 0.05
            three_target_y = two_target_y + 0.07
            four_target_y   = three_target_y + 0.04
            # five_target_y   = four_target_y + delta
            ...
            if cut_stage == 0 and abs(tip_y - one_target_y) <= 0.1 and reached_board :
                print(f"[CUT STAGE 1] env={e} | tip_y={tip_y:.3f} | target={one_target_y:.3f} | z={tip_z:.3f}")
                self._replace_with_single_mesh(
                    e,
                    "./mani_skill/assets/fruits/update/cucumber/multicut/4cut/cucumber_1cut_final.obj",
                    object_center_z,
                )
                #import pdb;pdb.set_trace()
                self._cut_count[e] = 1
                ...
                self._replace_with_single_mesh(
                    e,
                    "./mani_skill/assets/fruits/update/cucumber/multicut/4cut/cucumber_2cut_final.obj",
                    object_center_z,
                )
                self._cut_count[e] = 2
                ...
                self._replace_with_single_mesh(
                    e,
                    "./mani_skill/assets/fruits/update/cucumber/multicut/4cut/cucumber_3cut_final.obj",
                    object_center_z,
                )
                self._cut_count[e] = 3
                ...
                self._replace_with_single_mesh(
                    e,
                    "./mani_skill/assets/fruits/update/cucumber/multicut/4cut/cucumber_4cut_final.obj",
                    object_center_z,
                )
                self._cut_count[e] = 4
