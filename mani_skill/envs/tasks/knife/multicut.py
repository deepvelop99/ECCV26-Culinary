...
@register_env("multicut", max_episode_steps=1000)
class TableTopCuttingEnv(BaseEnv):
    ...
    def _load_scene(self, options: dict):
        ...
        for env_idx in range(self.num_envs):
            ...
            #path = "./mani_skill/assets/fruits/apple.obj" # apple
            #path = "./maniskill/ManiSkill/mani_skill/assets/fruits/apple.obj" # apple
            path = "/home/por1329/2025cvpr/ManiSkill/mani_skill/assets/fruits/peach.obj" # apple
            ...
            # === 절삭 비대상 (예: 접시, 장식 등) ===
            extra_path = "./mani_skill/assets/fruits/apple.obj"
            extra_builder = self.scene.create_actor_builder()
            extra_builder.set_scene_idxs([env_idx])
            extra_builder.add_visual_from_file(str(extra_path), scale=[1, 1, 1])
            extra_builder.add_sphere_collision(radius=1e-1000)
            extra_obj = extra_builder.build(name=f"extra_obj_{env_idx}")
            if not hasattr(self, "extra_objs"):
                self.extra_objs = []
            self.extra_objs.append(extra_obj)
        # buffers
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        for e in env_idx.tolist():
            ...
            if hasattr(self, "extra_objs"):
                extra_pose = Pose.create_from_pq(
                    [rand_x + 0.1, rand_y + 0.3, rand_z],  # 예: apple 옆에 고정
                    init_quat
                )
                self.extra_objs[e].set_pose(extra_pose)
    ...
    def _after_control_step(self):
        ...
        # 잘린 mesh 경로 (apple 예시, apple/apple 등 교체 가능)
        cut_paths = [
            "/home/por1329/2025cvpr/ManiSkill/mani_skill/assets/fruits/update/peach/peach_left.obj",
            "/home/por1329/2025cvpr/ManiSkill/mani_skill/assets/fruits/update/peach/peach_right.obj",
        ]
        ...
        for e in range(self.num_envs):
            ...
            if reached_board and (not self._already_cut[e]) and abs(tip_y - one_target_y) <= 1/10*object_width:
