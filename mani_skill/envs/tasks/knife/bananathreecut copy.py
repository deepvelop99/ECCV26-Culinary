...
@register_env("banana3cut", max_episode_steps=1000)
class TableTopCuttingEnv(BaseEnv):
    ...
    def _after_control_step(self):
        ...
        for e in range(self.num_envs):
            ...
            left_target_y = object_center_y - object_width / 4.0 + 0.01
            cente_target =object_center_y
            right_target_y = object_center_y + object_width / 4.0  + 0.01
            ...
            # ===== 1차 절삭 =====
            if cut_stage == 0 and abs(tip_y - left_target_y) <= 0.03 and reached_board:
                print(f"[CUT STAGE 1] env={e} | tip_y={tip_y:.3f} | target={left_target_y:.3f} | z={tip_z:.3f}")
                self._replace_with_single_mesh(
                    e,
                    "./mani_skill/assets/fruits/update/banana/multicut/3cut/banana_1cut_final.obj",
                    object_center_z,
                )
                self._cut_count[e] = 1
            elif cut_stage == 1 and abs(tip_y - cente_target) <= 0.03 and reached_board:
                print(f"[CUT STAGE 2] env={e} | tip_x={tip_y:.3f} | target={right_target_y:.3f} | z={tip_z:.3f}")
                self._replace_with_single_mesh(
                    e,
                    "./mani_skill/assets/fruits/update/banana/multicut/3cut/banana_2cut_final.obj",
                    object_center_z,
                )
                self._cut_count[e] = 2
            # ===== 2차 절삭 =====
            elif cut_stage == 2 and abs(tip_y - right_target_y) <= 0.03 and reached_board:
                print(f"[CUT STAGE 2] env={e} | tip_x={tip_y:.3f} | target={right_target_y:.3f} | z={tip_z:.3f}")
                self._replace_with_single_mesh(
                    e,
                    "./mani_skill/assets/fruits/update/banana/multicut/3cut/banana_3cut_final.obj",
                    object_center_z,
                )
                self._cut_count[e] = 3
