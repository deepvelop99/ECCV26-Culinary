...
@register_env("banana2cut", max_episode_steps=1000)
class TableTopCuttingEnv(BaseEnv):
    ...
    def _after_control_step(self):
        ...
        for e in range(self.num_envs):
            ...
            left_target_y = object_center_y - object_width / 4.0
            right_target_y = object_center_y + object_width / 4.0
            ...
            if cut_stage == 0 and abs(tip_y - left_target_y) <= 0.03 and reached_board:
                ...
                self._replace_with_single_mesh(
                    e,
                    "./mani_skill/assets/fruits/update/bananatwo/banana_1cut_final.obj",
                    object_center_z,
                )
                self._cut_count[e] = 1

            # ===== 2차 절삭 =====
            elif cut_stage == 1 and abs(tip_y - right_target_y) <= 0.03 and reached_board:
                print(f"[CUT STAGE 2] env={e} | tip_x={tip_y:.3f} | target={right_target_y:.3f} | z={tip_z:.3f}")
                self._replace_with_single_mesh(
                    e,
                    "./update/bananatwo/banana_2cut_final.obj",
                    object_center_z,
                )
                self._cut_count[e] = 2
