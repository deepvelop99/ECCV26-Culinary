...
@register_env("cucumber5cut", max_episode_steps=1000)
class TableTopCuttingEnv(BaseEnv):
    ...
    def _after_control_step(self):
        ...
        tip_p = self._get_tip_from_eef() # 0.1165, 0.1239, 0.4719
        # tip_p = self.agent.tcp.pose.p # -0.3623, 0.0308, 0.5968
        ...
        for e in range(self.num_envs):
            ...
            two_target_y  = one_target_y + 0.03
            three_target_y = object_center_y
            four_target_y   = three_target_y + 0.09
            five_target_y   = four_target_y + 0.02
            ...
            if cut_stage == 0 and abs(tip_y - one_target_y) <= 0.1 and reached_board :
                ...
                self._replace_with_single_mesh(
                    e,
                    "./mani_skill/assets/fruits/update/cucumber/multicut/5cut/cucumber_1cut_final.obj",
                    object_center_z,
                )
                self._cut_count[e] = 1
            elif cut_stage == 1 and abs(tip_y - two_target_y) <= 0.05 and reached_board:
                print(f"[CUT STAGE 2] env={e} | tip_x={tip_y:.3f} | target={two_target_y:.3f} | z={tip_z:.3f}")
                self._replace_with_single_mesh(
                    e,
                    "./mani_skill/assets/fruits/update/cucumber/multicut/5cut/cucumber_2cut_final.obj",
                    object_center_z,
                )
                self._cut_count[e] = 2
                ...
                self._replace_with_single_mesh(
                    e,
                    "./mani_skill/assets/fruits/update/cucumber/multicut/5cut/cucumber_3cut_final.obj",
                    object_center_z,
                )
                self._cut_count[e] = 3
            elif cut_stage == 3 and abs(tip_y - four_target_y) <= 0.025 and reached_board:
                print(f"[CUT STAGE 4] env={e} | tip_x={tip_y:.3f} | target={four_target_y:.3f} | z={tip_z:.3f}")
                self._replace_with_single_mesh(
                    e,
                    "./mani_skill/assets/fruits/update/cucumber/multicut/5cut/cucumber_4cut_final.obj",
                    object_center_z,
                )
                self._cut_count[e] = 4
                ...
                self._replace_with_single_mesh(
                    e,
                    "./mani_skill/assets/fruits/update/cucumber/multicut/5cut/cucumber_5cut_final.obj",
                    object_center_z,
                )
                self._cut_count[e] = 5
