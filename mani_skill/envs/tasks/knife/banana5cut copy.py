...
# 설정 / 경로 / 파라미터
# ==========================
KNIFE_STL_PATH = "/home/por1329/mani_skill/ManiSkill/mani_skill/assets/robots/panda/franka_description/meshes/visual/knifev2.stl"
HAND_TO_KNIFE_XYZ = (0.1, 0.0, 0.1)
HAND_TO_KNIFE_RPY = (1.5708, -1.5708, -1.5708)
KNIFE_LINK_NAME = "tool_knife"

@register_env("banana5cut", max_episode_steps=1000)
class TableTopCuttingEnv(BaseEnv):
    ...
    def _default_sensor_configs(self):
        ...
        return [CameraConfig("base_camera", pose=pose, width=512, height=512, fov=1.2, near=0.01, far=100)]
    ...
    def _default_human_render_camera_configs(self):
        return CameraConfig(
            ...
            # width=512, height=512, far=25, fov=0.63,
            width=512, height=512, far=10, fov=0.63,
        )
    ...
    def _load_scene(self, options: dict):
        ...
        for env_idx in range(self.num_envs):
            path = "./mani_skill/assets/fruits/banana.obj"
            ...
            self.block_apple.append(builder.build(name=f"block_banana_{env_idx}"))
    ...
    def _after_control_step(self):
        ...
        for e in range(self.num_envs):
            ...
            # inside_tip = (
            #     (tip_p[e, 1] > pose_p[1] - half[1]) & (tip_p[e, 1] < pose_p[1] + half[1]) &
            #     (tip_p[e, 2] > pose_p[2] - half[2]) & (tip_p[e, 2] < pose_p[2] + half[2])
            # )
            ...
            #print(contact[e])
            tip_y = tip_p[e, 1].item() 
            tip_z = tip_p[e, 2].item()
            board_z = self.board_center[2]
            cut_stage = int(self._cut_count[e])
            reached_board = tip_z <= board_z + 0.21
            ...
            delta = object_width / 5.0
            one_target_y  = object_center_y + 0.11
            two_target_y  = one_target_y - 0.04
            three_target_y = two_target_y - 0.07
            four_target_y   = three_target_y - 0.08
            five_target_y   = four_target_y - 0.04
            # one_target_y  = object_center_y + 2.0 * delta - 0.015
            #two_target_y  = object_center_y + 1.8 * delta - 0.05
            # three_target_y = object_center_y + 0.01
            # four_target_y   = object_center_y - 0.8 * delta - 0.03
            # five_target_y   = object_center_y - 1.0 * delta - 0.06
            # === 상태 출력 ===
            print(
                f"[STATE] env={e} | stage={cut_stage} "
                f"| object_center_y={object_center_y:.4f} "
                f"| delta={delta:.4f} "
                f"| one_target_y={one_target_y:.4f} "
                f"| two_target_y={two_target_y:.4f} "
                f"| three_target_y={three_target_y:.4f} "
                f"| four_target_y={four_target_y:.4f} "
                f"| five_target_y={five_target_y:.4f} "
                f"| tip_y={tip_y:.4f} "
                f"| tip_z={tip_z:.4f}"
            )           #     f"| left_target={left_target_y:.3f} | right_target={right_target_y:.3f} "
            #     f"| reached_board={reached_board}"
            # )
            # import pdb; pdb.set_trace()
            
            if cut_stage == 0 and abs(tip_y - one_target_y) <= 0.03 and reached_board:
                print(f"[CUT STAGE 1] env={e} | tip_y={tip_y:.3f} | target={one_target_y:.3f} | z={tip_z:.3f}")
                self._replace_with_single_mesh(
                    e,
                    "./mani_skill/assets/fruits/update/banana/multicut/5cut/banana_1cut_final.obj",
                    object_center_z,
                )
                #import pdb;pdb.set_trace()
                self._cut_count[e] = 1
            elif cut_stage == 1 and abs(tip_y - two_target_y) <= 0.03 and reached_board:
                print(f"[CUT STAGE 2] env={e} | tip_x={tip_y:.3f} | target={two_target_y:.3f} | z={tip_z:.3f}")
                self._replace_with_single_mesh(
                    e,
                    "./mani_skill/assets/fruits/update/banana/multicut/5cut/banana_2cut_final.obj",
                    object_center_z,
                )
                self._cut_count[e] = 2
                
            elif cut_stage == 2 and abs(tip_y - three_target_y) <= 0.03 and reached_board:
                print(f"[CUT STAGE 3] env={e} | tip_x={tip_y:.3f} | target={three_target_y:.3f} | z={tip_z:.3f}")
                self._replace_with_single_mesh(
                    e,
                    "./mani_skill/assets/fruits/update/banana/multicut/5cut/banana_3cut_final.obj",
                    object_center_z,
                )
                self._cut_count[e] = 3
            elif cut_stage == 3 and abs(tip_y - four_target_y) <= 0.03 and reached_board:
                print(f"[CUT STAGE 4] env={e} | tip_x={tip_y:.3f} | target={four_target_y:.3f} | z={tip_z:.3f}")
                self._replace_with_single_mesh(
                    e,
                    "./mani_skill/assets/fruits/update/banana/multicut/5cut/banana_4cut_final.obj",
                    object_center_z,
                )
                self._cut_count[e] = 4
            
            elif cut_stage == 4 and abs(tip_y - five_target_y) <= 0.02 and reached_board:
                print(f"[CUT STAGE 4] env={e} | tip_x={tip_y:.3f} | target={five_target_y:.3f} | z={tip_z:.3f}")
                self._replace_with_single_mesh(
                    e,
                    "./mani_skill/assets/fruits/update/banana/multicut/5cut/banana_5cut_final.obj",
                    object_center_z,
                )
                self._cut_count[e] = 5
