...
from mani_skill.utils.scene_builder.kitchen_counter.scene_builder import KitchenCounterSceneBuilder
...
@register_env("berrycut2", max_episode_steps=1000)
class TableTopCuttingEnv(BaseEnv):
    ...
    def _load_scene(self, options: dict):
        # Kitchen counter 기반 scene 로드
        self.kitchen_scene = KitchenCounterSceneBuilder(self, robot_init_qpos_noise=0)
        self.kitchen_scene.build(scale=1.0)

        # Board는 따로 안 올려도 되지만, 컷팅 테스트용 추가
        ...
        board_builder.initial_pose = sapien.Pose(p=[-0.1, 0, 0.92])  # kitchen counter top height
        ...
        for env_idx in range(self.num_envs):
            ...
            path = "./mani_skill/assets/fruits/strawberry.obj" # strawberry
    ...
    def _after_control_step(self):
        ...
        print("[contact]", self._contact_frame)  # 디버그 프린트

        # 잘린 mesh 경로 (strawberry 예시, apple/cucumber 등 교체 가능)
        cut_paths = [
            "./mani_skill/assets/fruits/update/strawberry/strawberry_left.obj",
            "./mani_skill/assets/fruits/update/strawberry/strawberry_right.obj",
        ]
        ...
        for e in range(self.num_envs):
            ...
            reached_board = tip_z <= board_z + 0.05   # 작은 margin 허용
    ...
    def get_state_dict(self) -> Dict:
        ...
        task["contact_enter"] = np.asarray(self._contact_enter_frame, dtype=np.bool_)
        task["contact_exit"]  = np.asarray(self._contact_exit_frame, dtype=np.bool_)
        state["task"] = task
    ...
    def _get_actor_aabb(self, actor):
        ...
        if mesh_path is None or not os.path.exists(mesh_path):
            mesh_path = "./mani_skill/assets/fruits/strawberry.obj"
