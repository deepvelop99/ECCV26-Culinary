"""ManiSkill ⇄ MPM bridge — addfruits-pilot variant (bridge5, stable).

Frozen snapshot of the bridge3 path after the v8 verification round
(2026-05-10). Public API identical to `bridge.py` / `bridge2` / `bridge3`.

Repo: imports the MPM core + assets from `/data/Cutting_rubuttal2` (same as
bridge3). Override `BridgeConfig.eef_repo` to swap to a different repo for
A/B tests.

What bridge5 includes vs the original bridge2/3 path:
  • mpm_extra_descent_y is 0 (any non-zero value pushes world_low_y below
    board_tolerance and triggers force_up() every substep, bouncing the
    knife above the fruit and preventing contact).
  • Knife XY taken from `_get_tip_from_eef()` (correct blade tip — blade is
    mounted ~0.30 m horizontally from the TCP). Knife Z taken from the TCP
    + calibrated `tcp_to_tip_z` (default −0.10) to avoid the FK matrix Z
    instability documented in bananacut.py.
  • Runtime canonical measurement: after seed_particles_from_mesh, the
    actual particle AABB midpoint is read and overwrites the static
    FRUIT_CANONICAL value so per-episode scale variation does not drift the
    knife off-center.
  • MPM↔MS coupling (knife.y AND knife.z_off) is gated on
    `_ms_driven_knife`. The collect script flips this to True only during
    the pure-Z cut phase; approach / hold / retreat keep coupling off so
    TCP rotation / lateral motion does not wiggle the MPM knife.

bridge3 stays as the live development copy; bridge5 is the version pinned
...
# Cutting_rubuttal2 — bridge3's default. Mirrors Cutting_rebuttal layout
# (mpmcore, sdf_utils, configs/fruits, assets/fruit2) but with the newer
# rebuttal-2 tweaks. Importing requires it on sys.path.
DEFAULT_REPO = Path("/data/Cutting_rubuttal2")
...
class BridgeConfig:
    ...
    # Fixed offset (m) from TCP to knife tip in world z during cutting.
    # Negative = tip is below TCP. Measured empirically: -0.101 m for pkour robot.
    tcp_to_tip_z: float = -0.10
...
class ManiSkillMPMBridge:
    ...
    def _load_mpm(self):
        ...
        try:
            ...
            # Runtime canonical measurement.
            # Fix 1: settle particles under gravity before measuring — the seed
            # function places particles inside the mesh, but they're not yet at
            # equilibrium. Without settle, the canonical reflects the seeded
            # (slightly above-board) state; particles then fall during the
            # episode and the knife XYZ ends up off-center as the fruit drops.
            # Fix 2: use particle centroid (mean) instead of AABB midpoint —
            # for asymmetric fruits (pear etc.) the AABB midpoint is offset
            # from the visible center of mass; the mean tracks the actual
            # particle distribution, so the knife lines up with the visible
            # fruit center.
            SETTLE_STEPS = 80
            try:
                t = 0.0
                ...
                for _ in range(SETTLE_STEPS):
                    self.sim.step(t)
                    t += dt_frame
                arr = self.sim.particles.to_numpy()["x"]
                if arr.shape[0] > 0:
                    mid = arr.mean(axis=0)             # centroid (mass-weighted)
                    lb  = arr.min(axis=0)
                    ub  = arr.max(axis=0)
                    aabb_mid = (lb + ub) / 2.0
                    self.calib.mpm_banana_canonical_xz = (float(mid[0]), float(mid[2]))
                    self.calib.mpm_banana_canonical_y  = float(mid[1])
                    print(f"[bridge5] runtime canonical (N={arr.shape[0]} after "
                          f"{SETTLE_STEPS} settle steps): "
                          f"centroid_xz=({mid[0]:+.5f}, {mid[2]:+.5f}) y={mid[1]:.5f}  "
                          f"aabb_mid_xz=({aabb_mid[0]:+.5f}, {aabb_mid[2]:+.5f}) y={aabb_mid[1]:.5f}  "
                          f"(was {self.calib.fruit} static)")
                    # Reset accumulated MPM time so step_with_env starts at 0.
                    self._mpm_sim_time = 0.0
            except Exception as e:
                print(f"[bridge5] runtime canonical measurement skipped: {e}")
        finally:
            os.chdir(prev_cwd)
    ...
    def setup(self, env, *, banana_xy_ms=None, banana_yaw_ms: float = 0.0):
        ...
        # Read the env-exposed cut surface so V_OFFSET tracks the bg variant
        # (board top z=0.020 vs no-board z=0.000). Falls back to the legacy
        # 0.020 when the env doesn't expose `cut_surface_z` (vanilla bananacut).
        # MPM board top is fixed at MPM_BOARD_TOP_Y=0.04642 regardless of bg.
        cut_surface_z_ms = float(getattr(u, "cut_surface_z", 0.020))
        # Lazy import to avoid pulling MPM_BOARD_TOP_Y into module init order.
        from .frame import MPM_BOARD_TOP_Y as _MPM_BTY
        v_off_dyn = float(_MPM_BTY) - cut_surface_z_ms
        self.calib.v_offset = v_off_dyn
        print(f"[bridge5] cut_surface_z(ms)={cut_surface_z_ms:.4f}  "
              f"v_offset = MPM_BOARD_TOP_Y({_MPM_BTY:.4f}) - cut_surface_z = "
              f"{v_off_dyn:.4f}")
    ...
    def step_with_env(self, env, dt_control: float) -> Dict[str, np.ndarray]:
        ...
        # Use _get_tip_from_eef() for XY (accurate blade lateral position — blade is
        # ~0.30 m horizontally from TCP, so TCP XY is NOT the blade position).
        # For Z use TCP + calibrated offset: avoids Z instability from eef matrix math
        # under rapid joint motion (see bananacut.py). tcp_to_tip_z ≈ -0.05 to -0.10 m.
        TCP_TO_TIP_Z = float(getattr(self.cfg, "tcp_to_tip_z", -0.05))
        tcp_pos = np.asarray(u.agent.tcp.pose.p, np.float32).reshape(-1)
        try:
            tip_ms_eef = u._get_tip_from_eef()[0].detach().cpu().numpy().astype(np.float32)
            tip_ms = np.array([
                float(tip_ms_eef[0]),
                float(tip_ms_eef[1]),
                float(tcp_pos[2]) + TCP_TO_TIP_Z,
            ], np.float32)
        except Exception:
            tip_ms = tcp_pos.copy()
            tip_ms[2] += TCP_TO_TIP_Z
        ...
        # Gate BOTH knife.y and knife.z_off updates on _ms_driven_knife.
        # MPM↔MS coupling should ONLY apply during the pure-Z descent (cut
        # phase). During approach/hold/retreat the TCP rotates / translates
        # laterally; if we kept updating z_off and knife.y from those frames
        # the MPM knife wiggles around (and may sweep through the seeded fruit
        # before the cut even starts). With the gate, the lateral knife
        # position is fixed at whatever it was at the last MS-driven step
        # (typically the start of cut, when collect calls set_ms_driven(True)).
