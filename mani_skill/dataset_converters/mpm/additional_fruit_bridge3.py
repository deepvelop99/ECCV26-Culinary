"""ManiSkill ⇄ MPM bridge — addfruits-pilot variant (bridge3).

Public API identical to `bridge.py` / `additional_fruit_bridge2.py`. The
only difference vs bridge2 is the default repo: bridge3 imports the MPM
core + assets from `/data/Cutting_rubuttal2` (the second-iteration rebuttal
fork — newer mpmcore, additional config tweaks). Override
`BridgeConfig.eef_repo` to swap to a different repo for A/B tests.

bridge2 ↔ bridge3 are kept side-by-side so we can compare cut behaviour
between the two MPM cores without touching the legacy bridge.py path.
"""
...
class ManiSkillMPMBridge:
    ...
    def _load_mpm(self):
        ...
        try:
            ...
            # Runtime canonical measurement: with fruit_scale_override applied
            # per episode, the static FRUIT_CANONICAL (measured at yaml-default
            # scale) drifts as scale varies. Read the actual particle AABB
            # midpoint here and overwrite calibration so the bridge always
            # places the MPM knife exactly on the seeded fruit center.
            try:
                ...
                if arr.shape[0] > 0:
                    ...
                    print(f"[bridge3] runtime canonical (N={arr.shape[0]}): "
                          f"xz=({mid[0]:+.5f}, {mid[2]:+.5f})  y={mid[1]:.5f}  "
                          f"(was {self.calib.fruit} static)")
            except Exception as e:
                print(f"[bridge3] runtime canonical measurement skipped: {e}")
        finally:
            os.chdir(prev_cwd)
